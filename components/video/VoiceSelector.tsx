...
export function VoiceSelector({ selectedVoice, onVoiceChange, voices }: VoiceSelectorProps) {
  const { theme } = useTheme();
  
  // Use API voices if provided, otherwise fallback to mock data
  const displayVoices = voices && voices.length > 0 ? voices : voiceTypes;
    ...
    <div className="mb-4 sm:mb-5 md:mb-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2 sm:gap-3">
        {displayVoices.map((voice) => {
          // Check if it's an API voice or mock voice
          const hasAvatar = 'avatar' in voice;
